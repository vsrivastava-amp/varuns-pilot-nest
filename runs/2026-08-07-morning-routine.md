...
## Second pass — Slack coverage gap (Varun caught this)

- 2026-08-07 ~10:53 — Varun asked "did you check slack". The honest answer was yes-but-partially: the first pass read `#pub-onboarding-qwant-ai`, `#team-relevance-yield`, `#online-civ-service`, and `#proj-query-civ-extraction`, plus the DM sweep and keyword searches, and **skipped four anchors from the playbook's own list** (`#prod-relevance-yield-alerts`, `#gongtest`, `#vespa-optimizations-xfn`, `#proj-scale-serving-and-tracking`) and the own-messages sweep. The digest as first written did not disclose the gap, which is the worse half of the mistake.
- 2026-08-07 — **Closing the gap changed a flag.** `#proj-scale-serving-and-tracking` carried Roberto's filter-distribution measurement over the real Qwant dataset: of 9,533 pCIV targets, exactly one has all six filters, and price filters (the expensive kind) appear on under 2%. That retires the 8/05 "we can't go live with these filters" concern, and it is measured on this stream's own output. `#vespa-optimizations-xfn` carried the unresolved 200x profiler-vs-load-test discrepancy on `casual shoe`, which means no Vespa latency number is currently safe to quote. Neither would have been found by the first pass.
- 2026-08-07 — Digest header now states the Slack coverage list explicitly, including what was not read, and says the second pass happened. Three new gotchas folded into `playbooks/morning-routine.md`: the `from:` sweep forms that silently fail, the `#gongtest` double-truncation (tool-result overflow *and* a silent older-end cutoff needing a second bounded read), and the Gong room-attribution rate.
- 2026-08-07 — Lesson worth keeping: **a sweep that does not publish its own coverage list cannot be audited, by Varun or by the next session.** Name the channels read and the ones skipped, every time.

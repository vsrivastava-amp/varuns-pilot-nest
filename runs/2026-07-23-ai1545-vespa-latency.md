...
- 2026-07-24 Overnight thread updates digested for Varun: Dhaval demanded prod numbers (22:07 7/23) → Artem ran a prod probe through prod VSS (4:32 ET): prod VSS→Vespa timeout = **150ms** (cd-deploy-configs), 2^8 filter-combo sweep on "adidas shoes" limit 10. Heavy filters = GPC, currencyCodes, minPrice, maxPrice; **GPC-or-currency + any other heavy filter → exceeds 150ms on prod**. Aligns with our stage findings + price no-fast-search landmine; NEW surprise: currencyCodes heavy despite fast-search+rank:filter in canonical schema (near-universal filter cost at prod scale — mechanism unconfirmed). Artem confirmed 0-ads expected (CIV "Minnie Mouse" vs ad "Disney" = contradiction excluded); AAS sends one VSS request per intentTarget. Dhaval wants stage-style avg/p99 table for prod + eventually e2e via SSP ("true number that matters for pubs"). No Varun action pending.

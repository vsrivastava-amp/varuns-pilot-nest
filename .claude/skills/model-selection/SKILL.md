...
- **Mantle ≠ OpenAI-only (corrected 2026-07-28):** Project Mantle is AWS's serverless onboarding rail for new models generally — it also serves open-weight models (DeepSeek V3.2, MiniMax M2.1, GLM 4.7/Flash, Kimi K2.5, Qwen3 Coder Next since Feb 2026; Gemma 4 since June 2026 — Gemma 4 is the only open model with prompt caching, implicit/automatic; latency benefit documented, price discount unconfirmed). The bedrock-mantle endpoint speaks OpenAI Responses, OpenAI Chat Completions, AND Anthropic Messages APIs — but per-model support varies (GPT-5.6 family is Responses-only). Definitive per-model surface/API answer: the model's card under https://docs.aws.amazon.com/bedrock/latest/userguide/model-cards.html (there is NO `/models` listing endpoint on Mantle — GET 404s; verified 2026-07-28). **Mantle has TWO API roots** (verified live 2026-07-28): `…api.aws/openai/v1/*` (OpenAI closed-weight + Gemma 4) and `…api.aws/v1/*` (the open-weight cohort's Chat Completions — the same models as bedrock-runtime, sometimes under a different id, e.g. `qwen.qwen3-next-80b-a3b-instruct` on Mantle vs `qwen.qwen3-next-80b-a3b` on runtime). A "isn't supported on this route" 400 means try the other root; "does not exist" means wrong id. Auth for ALL Mantle calls: Bedrock API key (short-term keys derive from the current IAM principal via `aws-bedrock-token-generator`, ≤12h, auto-refresh — IRSA-compatible); IAM action `bedrock-mantle:CallWithBearerToken` (distinct from `bedrock:*`).

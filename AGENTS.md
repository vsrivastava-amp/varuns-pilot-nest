...
3. Skim `tasks/queue.md` (open work + claims) and `ls review/` (pending outbound drafts — **flag any pending ones to Varun in-chat**).
...
| `review/` | Outbound drafts awaiting Varun — one bare file per draft, exactly what gets pasted/executed (`review/README.md` has conventions); context goes in your `runs/` entry; disposition happens in-chat | Add file; delete on disposal (git keeps history) |
...
- Every line you write in `log/`, `tasks/`, `runs/` starts with a date (YYYY-MM-DD); `review/` drafts carry the date in the filename instead (bodies stay bare).
...
2. **Never send messages as Varun** — drafts only, into `review/`.
...
6. Outbound artifacts (messages, PRs beyond dev branches, Jira writes) go through `review/` drafts. Keep working other tasks while they wait.
7. Ambiguous → park your best-guess draft in `review/`. Don't guess on anything outward-facing.
...
- 2026-07-22 — claude.ai MCP connectors live on laptop sessions: Calendar, Gmail, Drive, Atlassian Rovo, Slack (per-session `/mcp` handshake required — see `playbooks/google.md`). **Slack Claude retired** (see `log/nest--laptop.md`); morning routine v2 runs all sweeps laptop-side (`playbooks/morning-routine.md`). ~~Still pending: Datadog keys.~~ 2026-07-27: superseded — see the Datadog line below.
...
- 2026-07-27 — **REVIEW.md deprecated** (Varun-directed): `review/` is the canonical draft gateway — bare files, context in `runs/`, dispositions in-chat, delete on disposal. See `review/README.md`. Old REVIEW.md is a tombstone until next steward compaction.
- 2026-07-27 — **Datadog: there are no keys to wait for.** Access is a **claude.ai MCP connector** ("claude.ai Datadog", `mcp.datadoghq.com`), same shape as Calendar/Gmail/Drive/Rovo: per-session `/mcp` handshake, Varun's OAuth, read-only by policy (guardrail #8). Don't file an admin ask for API keys. Before the handshake only `authenticate`/`complete_authentication` are exposed and every real tool is invisible — that is the un-handshaken state, not a missing capability. Ask Varun for `/mcp`. See `playbooks/datadog.md`.

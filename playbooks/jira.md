...
- 2026-08-07 — **The subagent-curl gotcha above did NOT reproduce.** A Jira subagent ran the credentialed curl `/search/jql` path end to end today: 168 issues over 2 pages, 40 created-in-window reconciled against the `NEW` flags, 195 per-issue comment fetches, no classifier refusal and no silent failure. Because the earlier gotcha describes a *silent* failure that looks like success, the main session spot-checked five specific claims from the agent's report before publishing (DPR-3420, AS-13435, INFRA-3507, INFRA-3508, AI-1659) — status, assignee, and summary matched on all five. So: the denial is conditional, not universal. Keep giving subagents the curl path, and keep spot-checking a handful of their concrete claims from the main session — that check is cheap and it is the only thing that distinguishes a real sweep from a fabricated one.
- 2026-08-07 — **Read the Scope/`customfield_11292` field, not just the summary and description, on DPR config tickets.** DPR-3420's summary said `civExtraction`, its description said `/v1/intent/pciv`, and its Scope field said `(key: pcivExtraction)` with only the display name struck through — three different answers on one ticket, and the Scope field is the one that describes what actually gets built. `fields=*all` with `expand=renderedFields` is the cheap way to see it; the plain `description` fetch misses it entirely.

...
- **A Delta-row retry may still be a service-cache hit**: deleting a notebook output row only defeats the notebook's anti-join. If the service caches by eval ID + normalized query, the rerun can reproduce the old result without invoking the model. For a real re-evaluation, use the API's optional `bypassCache: true` on the scoped retry and verify gateway traffic. AI-1474 proof: 762 apparent retry rows produced zero model calls until bypass was enabled; the bypass run recovered 337 categories.
...
- **Serverless compute can't reach VPC-internal services**: no private DNS (`*.ric1.admarketplace.net` → "Name or service not known") and no VPC peering. Classic clusters run in the workspace VPC and resolve/route fine — switch cluster, don't debug DNS. Partial workaround if stuck on serverless: internal ELB AWS hostnames ARE publicly resolvable (return 10.9.x) + send the ingress hostname as an explicit `Host:` header — but the route still needs VPC connectivity, so this only helps classic clusters with the split-horizon DNS gap.- **`jobs cancel-run` is not a reliable kill for notebook tasks on shared all-purpose clusters** — the run goes TERMINATED/CANCELED but the notebook command can keep executing as a zombie (observed twice on AI-1474: kept writing Delta rows 1h+ after cancellation; a cancelled async pipeline kept hammering the AI gateway). Detect: query the output table for writes by the cancelled run_id after cancellation. Guaranteed kill: `databricks clusters restart <id>` (check `jobs list-runs --active-only` first for collateral — job-cluster runs are unaffected).

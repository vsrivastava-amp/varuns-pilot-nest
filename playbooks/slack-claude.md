...
Rules for the applying session: read the diff before applying — deposits touch only Slack-Claude-owned files per the single-writer rule; anything touching `map/`, `PLAN.md`, `CLAUDE.md`, or playbooks is a *proposal* and goes to `review/` instead of direct apply.

...
- [x] **762-keyword targeted re-run COMPLETE 2026-07-30** (promised to Dhaval on-ticket "later this week"): target = 762 rows / 762 distinct keywords, classified 2026-07-27 16:54:10–16:56:34 UTC across 28 storm minutes. First retry exposed a service-cache gotcha: all 762 were eval-6 DynamoDB hits and never reached Gemini. Added optional notebook widget `bypass_cache` (default false), passed it as API `bypassCache`, and reran exactly 762 at concurrency 3 as Databricks run `52460964734863` / table run ID `gemini-storm-retry-2`. Result: 337 gained categories, 307 clean empty classifications, 118 exhausted five service-level retries; gateway window had 190 requests, all HTTP 200 and zero 429s. Final eval-6 table = 2,422,276 rows / 2,422,276 distinct keywords / zero duplicates; residual retry failures = 0.0049% of table. Full 460k retry_empty not needed — Dhaval accepted 0.03%.
...
- [x] **Flag Sunil first**: Varun sent an edited message on 2026-07-30 asking how to reconcile the prod `civ_config` table with service-only evals 5/6, which ID Gemini should use, whether a reused cache namespace must be cleared, and whether prod capacity must be confirmed. Review draft disposed after send.
...
- **Context for pickup**: winning code = `feat-civ-eval-6-gemini` @ ae1da11 (llm-evaluator-service, Bitbucket) — eval 6 = gemini-3-6-flash + registry + gemini parse fixes; dev pod runs its image 1.0.288. Notebook (all fixes: status-aware ladder, slot-held backpressure, continuous pipeline, DNS fallback, optional service-cache bypass) lives in the dev workspace at /Users/vsrivastava@admarketplace.com/AI-1474_keyword_gpc_reclassification/L3_categorization_v2; deliverable table adv_keyword_gpc_lvl_3_ai1474 (2,422,276 distinct kws; zero duplicates after targeted retry); Emily has the path, BI/Chris informed. Full narrative: runs/2026-07-22-ai1474-scope.md. Ops lessons in playbooks/databricks.md + release-process skill.

...
- [!] **Tribikram instruction BLOCKED at `Q-2026-07-30-02`**: fresh 2026-07-30 Bitbucket history shows Tribikram's latest merged service PR is #44 / `9fa18ec` (AI-1361, 2026-07-21), already contained in both main and the Gemini branch. Varun must confirm whether "a certain regression needs to be made to main" means a regression fix/revert or a regression test pass.
...
- [ ] Full release process per `.claude/skills/release-process/SKILL.md` (RELEASE ticket, #releases, prod-branch CD PR + peer approval + Varun merge); sync `llm_evals.civ_config` row. **STOPPED 2026-07-30 before ticket/PR creation** pending Sunil's config-ID/cache decision and `Q-2026-07-30-02`. Exact remote commit `ae1da11` passes 307/307 tests on Python 3.13.1.

...
- [ ] **Early-testing model recommendation (updated)**: **`openai.gpt-5.6-luna`** = the nano-role model, cached, in-region — and it's *already in the service's eval configs via DBX* (evalId 6, gpt-5-6-luna), so accuracy/functional testing can start on the DBX path today while Mantle wiring lands. Bedrock-runtime alternates for the Converse path: `gpt-oss-20b` / `ministral-3b`.
